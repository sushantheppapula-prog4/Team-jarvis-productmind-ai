...
export default async function ProductWorkspace({ params }: { params: { id: string } }) {
  ...
  return <div className="p-8 lg:p-12 min-h-screen bg-[#F9F9F7]">
    ...
    <div className="mb-12 border-t-2 border-[#111111] pt-8"><p className="mb-5 font-mono text-[10px] font-bold uppercase tracking-[0.25em] text-[#CC0000]">Intelligence modules</p><nav className="grid grid-cols-1 gap-0 sm:grid-cols-2 lg:grid-cols-4">{[["Market Suggestion", "market-suggestion"], ["Review Report", "review-report"], ["Performance", "performance"], ["Scalability", "scalability"], ["Improvements", "improvements"], ["Next Generation", "next-generation"], ["New Product", "new-product-suggestions"], ["Continuous Reports", "continuous-reports"], ["AI Agent", "ai-agent"]].map(([label, slug]) => <Link key={slug} href={`/dashboard/product/${product.id}/${slug}`} className="border-2 border-[#111111] p-4 font-mono text-xs font-bold uppercase tracking-widest hover:bg-[#111111] hover:text-[#F9F9F7]">{label}<ArrowRight className="ml-2 inline h-3 w-3" /></Link>)}</nav></div>
