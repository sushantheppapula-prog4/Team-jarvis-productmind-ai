...
export async function analyzeReviews(productId: string) {
  ...
  if (research.status === "LIVE_REVIEW_RESEARCH_UNAVAILABLE") { await update("RESEARCH_PROVIDER_UNAVAILABLE", "LIVE REVIEW RESEARCH UNAVAILABLE"); return { report: null, status: "RESEARCH_PROVIDER_UNAVAILABLE" as const, message: "LIVE REVIEW RESEARCH UNAVAILABLE — Live customer evidence could not be retrieved at this time." }; }
  if (research.status === "NO_REVIEWS_FOUND") { await update("INSUFFICIENT_REVIEW_DATA", "NO REVIEWS FOUND"); return { report: null, status: "INSUFFICIENT_REVIEW_DATA" as const, message: "INSUFFICIENT LIVE REVIEW EVIDENCE — No validated customer review evidence was found." }; }
