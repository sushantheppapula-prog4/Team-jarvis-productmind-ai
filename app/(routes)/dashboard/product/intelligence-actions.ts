...
export async function getReportSchedule(productId: string) { const supabase = await createClient(); const { data: { user } } = await supabase.auth.getUser(); if (!user) return null; const { data } = await supabase.from("product_report_schedules").select("*").eq("product_id", productId).eq("user_id", user.id).maybeSingle(); return data; }
